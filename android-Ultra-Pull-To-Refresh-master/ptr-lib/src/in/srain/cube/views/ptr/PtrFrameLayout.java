...
public class PtrFrameLayout extends ViewGroup {
    ...
    private PtrUIHandlerHolderFoot ptrUIHandlerHolderFoot = PtrUIHandlerHolderFoot.create();
...
//            }
            ...
            if (mContent == null || mHeaderView == null || mFootView == null) {
                for (int i = 0; i < childCount; i++) {
                    View child = getChildAt(i);
                    if (child instanceof PtrUIHandlerFoot) {
                        mFootView = child;
                    } else if (child instanceof PtrUIHandler) {
                        mHeaderView = child;
                    } else {
                        mContent = child;
                    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mFootView != null) {
            ...
            mFootHeight = mFootView.getMeasuredHeight() + lp.topMargin + lp.bottomMargin;
            mPtrIndicator.setFootHeight(mFootHeight);
        }
    ...
    private void layoutChildren() {
        ...
        if (mFootView != null) {
            MarginLayoutParams lp = (MarginLayoutParams) mFootView.getLayoutParams();
            final int left = paddingLeft + lp.leftMargin;
            final  int   top = 50+ mContent.getMeasuredHeight() + paddingTop + lp.topMargin + offset;
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                boolean moveDown = offsetY < 0 || offsetY > 0;
...
//        }
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT) ||
            ...
            if (change < 0) {
                ptrUIHandlerHolderFoot.onUIRefreshPrepare(this);
            }
            if (DEBUG) {
        ...
        if (mPtrUIHandlerHolder.hasHandler() && moveY > 0) {
            mPtrUIHandlerHolder.onUIPositionChange(this, isUnderTouch, mStatus, mPtrIndicator);
        }
        if (ptrUIHandlerHolderFoot.hasHandler() && moveY < 0) {
            ptrUIHandlerHolderFoot.onUIPositionChange(this, isUnderTouch, mStatus, mPtrIndicator);
        }
        ...
        if (mStatus == PTR_STATUS_LOADING) {
            ...
            if (mKeepHeaderWhenRefresh) {
                ...
                if (mPtrIndicator.isOverOffsetToKeepHeaderWhileLoading() && !stayForLoading) {
                    ...
                    if (moveY < 0) {
                        mScrollChecker.tryToScrollTo(mPtrIndicator.getOffsetToKeepHeaderWhileLoadingFoot(), mDurationToClose);
                    }
                } else {
                    // do nothing
                }
    ...
    private void performRefresh() {
        ...
        if (moveY < 0) {
            if (ptrUIHandlerHolderFoot.hasHandler()) {
                ptrUIHandlerHolderFoot.onUIRefreshBegin(this);
                if (DEBUG) {
                    PtrCLog.i(LOG_TAG, "PtrUIHandler: onUIRefreshBegin");
                }
            }
    ...
    private void notifyUIRefreshComplete(boolean ignoreHook) {
        ...
        if (moveY < 0) {
            if (ptrUIHandlerHolderFoot.hasHandler()) {
                ...
                ptrUIHandlerHolderFoot.onUIRefreshComplete(this);
            }
        }
    ...
    public void addPtrUIHandlerFoot(PtrUIHandlerFoot ptrUIHandlerFoot) {
        PtrUIHandlerHolderFoot.addHandler(ptrUIHandlerHolderFoot, ptrUIHandlerFoot);
    }
    ...
    public void setFootView(View header) {
        if (mFootView != null && header != null && mFootView != header) {
